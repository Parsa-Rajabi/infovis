class SunBurst{
   ...
   vis.format = d3.format(",d")
    ...
    vis.MARGIN = {LEFT:60,RIGHT:30,TOP:50,BOTTOM:50};
    vis.width= 600 -vis.MARGIN.LEFT - vis.MARGIN.RIGHT;
    vis.height= 600 -vis.MARGIN.TOP - vis.MARGIN.BOTTOM;
    vis.radius = vis.config.containerWidth / 14



    vis.svg = d3.create("svg")
        // .attr("viewBox", [50, 80, vis.width, vis.height])
    ...
    updateVis() {
        ...
        vis.color = d3.scaleOrdinal(d3.quantize(d3.interpolateReds, vis.root.children.length +1))
    ...
    .outerRadius(d => Math.max(d.y0 * vis.radius, d.y1 * vis.radius - 1))
      ...
      .join("path")
        ...
        .attr("d", d => vis.arc(d.current)).attr('stroke','red')
        .attr('stroke-width','0.05px')
        .on('mouseover',function(event,i){
          vis.tooltip.transition().duration(200).style("opacity",0.7);
                vis.tooltip.html("<p>Name: "+i.data.id+"</p>").style("left",(event.pageX-700)+"px").style("top",(event.pageY-100)+"px");
